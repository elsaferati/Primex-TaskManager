...
Set all variables documented in `.env.example`. Gmail OAuth requires the `gmail.send` and read metadata/search access needed for Sent Mail verification. The sender must be the OAuth account (or an authorized alias). Secrets must remain in GitHub/PM2 configuration.
