...
def test_em_task_gets_one_0800_prefix() -> None:
    ...
    assert normalize_email_task_title("SYS: EM TEST", is_system_task=True) == "SYS: EM TEST"
...
def test_em_and_literal_time_are_both_0800_indicators() -> None:
    ...
    assert not title_has_eight_am_indicator("SYS: EM TEST", is_system_task=True)
    assert title_has_eight_am_indicator("SYS: 08:00 TEST", is_system_task=True)
