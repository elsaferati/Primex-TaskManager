...
def test_today_at_1600_rolls_to_next_day():
    assert effective_slot_date(MON, at(MON, 16, 0)) == TUE


def test_today_after_1600_rolls_to_next_day():
    assert effective_slot_date(MON, at(MON, 18, 30)) == TUE


def test_friday_after_1600_rolls_to_monday():
    assert effective_slot_date(FRI, at(FRI, 16, 0)) == NEXT_MON


def test_saturday_after_1600_rolls_to_monday():
    assert effective_slot_date(SAT, at(SAT, 16, 0)) == NEXT_MON


def test_sunday_after_1600_rolls_to_monday():
    assert effective_slot_date(SUN, at(SUN, 17, 0)) == NEXT_MON
