...
class EndWeekBzReportSettings(Base):
    __tablename__ = "end_week_bz_report_settings"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    is_active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    send_time: Mapped[time] = mapped_column(Time, nullable=False, default=lambda: time(16, 30))
    timezone: Mapped[str] = mapped_column(String(80), nullable=False, default="Europe/Tirane")
    weekdays: Mapped[list[int]] = mapped_column(ARRAY(Integer), nullable=False, default=lambda: [4])
