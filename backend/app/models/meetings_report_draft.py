...
class MeetingsReportDraft(Base):
    __tablename__ = "meetings_report_drafts"
    __table_args__ = (
        UniqueConstraint("report_date", name="uq_meetings_report_draft_report_date"),
    )

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    report_date: Mapped[date] = mapped_column(Date, nullable=False, index=True)
    tomorrow_date: Mapped[date] = mapped_column(Date, nullable=False)
