...
export default function ReportManagementPage() {
  ...
  const restoreDefaults=async(kind:"recipients"|"schedules")=>{if(!window.confirm(`Restore the default ${kind}?`))return;const res=await apiFetch(`${API}/${kind}/restore-defaults`,{method:"POST"});if(res.ok){toast.success(`Default ${kind} restored`);await load()}else toast.error("Restore failed",{description:await res.text()})}
  ...
  return <div className="mx-auto max-w-[1500px] space-y-5">
    ...
    <Tabs defaultValue="overview"><TabsList className="flex h-auto flex-wrap"><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="reports">Reports</TabsTrigger><TabsTrigger value="schedules">Schedules</TabsTrigger><TabsTrigger value="recipients">Recipients</TabsTrigger><TabsTrigger value="history">History</TabsTrigger><TabsTrigger value="settings">Settings</TabsTrigger></TabsList>
      <TabsContent value="overview"><div className="grid gap-4 md:grid-cols-3">{[["Active recipients",active.length],["Active schedules",schedules.filter(s=>s.is_active).length],["Recent successful runs",runs.filter(r=>["SENT","ALREADY_SENT"].includes(r.status)).length]].map(([label,value])=><div className="rounded-xl border p-5" key={String(label)}><p className="text-sm text-muted-foreground">{label}</p><p className="mt-2 text-3xl font-semibold">{value}</p></div>)}</div></TabsContent>
