...
export default function DepartmentKanban() {
  ...
  const allTodayPrintItems = React.useMemo(() => {
    const items: Array<{ userId: string; period: "AM" | "PM"; label: string }> = []
    for (const task of todayProjectTasks) {
      ...
      items.push({
        userId,
        period,
        label: `Project: ${projectLabel} - ${task.title}`,
      })
    }
    for (const task of todayNoProjectTasks) {
      ...
      items.push({
        userId,
        period,
        label: `${noProjectTypeLabel(task)}: ${task.title}`,
      })
    }
    for (const task of todaySystemTasks) {
      ...
      items.push({
        userId,
        period,
        label: `System: ${task.title || "System task"}`,
      })
    }
    ...
    for (const meeting of todayMeetings) {
      ...
      items.push({
        userId,
        period,
        label: `Meeting: ${formatMeetingPrintLabel(meeting)}`,
      })
    }
    return items
  ...
  const allTodayPrintColumns = React.useMemo(() => {
    ...
    return columns
  }, [allTodayPrintBaseUsers, allTodayPrintHasUnassigned])

  const allTodayPrintByUser = React.useMemo(() => {
    const map = new Map<string, { AM: string[]; PM: string[] }>()
    const ensure = (userId: string) => {
      if (!map.has(userId)) {
        map.set(userId, { AM: [], PM: [] })
      }
    ...
    for (const item of allTodayPrintItems) {
      ...
      bucket[item.period].push(item.label)
    }
    for (const bucket of map.values()) {
      bucket.AM.sort((a, b) => a.localeCompare(b))
      bucket.PM.sort((a, b) => a.localeCompare(b))
    }
    return map
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {viewMode === "department" ? (
                  <>
                    ...
                    <Button
                      variant="ghost"
                      className="h-9 rounded-xl px-3 text-sm text-slate-700 hover:bg-slate-100"
            ...
            <table className="w-full border border-slate-900 text-[11px]">
              <thead>
                <tr className="bg-slate-100">
                  ...
                  <th className="border border-slate-900 px-2 py-2 text-left text-xs uppercase">Time</th>
              ...
              <tbody>
                {(["AM", "PM"] as const).map((period, periodIndex) => (
                  <tr key={period}>
                    {periodIndex === 0 ? (
                      <td rowSpan={2} className="border border-slate-900 px-2 py-2 align-top font-semibold uppercase">
                        {formatPrintDay(todayDate)}
                      </td>
                    ) : null}
                    <td className="border border-slate-900 px-2 py-2 align-top font-semibold uppercase">{period}</td>
                    {allTodayPrintColumns.map((column) => {
                      const bucket = allTodayPrintByUser.get(column.id)
                      const items = bucket ? bucket[period] : []
                      return (
                        <td key={`${column.id}-${period}`} className="border border-slate-900 px-2 py-2 align-top">
                          {items.length ? (
                            <div className="space-y-1">
                              {items.map((item, itemIndex) => (
                                <div
                                  key={`${column.id}-${period}-${itemIndex}`}
