...
const outcomeLabel: Record<string, string> = {
  ...
  REOPENED: "Rihapur", REMOVED_FROM_PLAN: "Hequr nga plani", REASSIGNED_OUT: "Transferuar jashtë",
...
export function DailyRealizationView() {
  ...
  const tasks = (selected?.tasks || []).filter(task =>
    ...
    (!exceptionsOnly || task.issues.length > 0 || ["NO_PROGRESS", "BLOCKED", "REOPENED", "REASSIGNED_OUT", "REASSIGNED_IN", "REMOVED_FROM_PLAN", "POSTPONED_UNAPPROVED", "POSTPONED_APPROVED"].includes(task.classification))
  )
  ...
  return <div className="space-y-4">
    ...
    {data ? <Kpis metrics={selected?.metrics || data.metrics} /> : null}
